...
class CRCServer(object):
    ...
    def __init__(self, options, run_on_localhost=False):
        ...
        # DO NOT CHANGE THE CONTENTS OF THIS VARIaBLE YOURSELF
        # -----------------------------------------------------------------------------
    ...
    def setup_server_socket(self):
        ...
        self.sel.register(server, event, data=None)
    ...
    def connect_to_server(self):
        ...
        server = socket(AF_INET, SOCK_STREAM)
        server.setblocking(False)
        server.connect_ex((self.connect_to_host_addr, self.connect_to_port))
        ...
        self.sel.register(server,event, data=data)
    ...
    def check_IO_devices_for_messages(self):
        ...
        while not self.request_terminate:
            # TODO: Implement the above functionality
            read_IO = self.sel.select(timeout=0.1)

            for io_device, event_mask in read_IO:
    ...
    def cleanup(self):
        ...
        for x in keys:#range(len(keys)):
            self.sel.unregister(x.fileobj)
            x.fileobj.close()
    ...
    def accept_new_connection(self, select_key):
        ...
        datas = ConnectionData()
        event = selectors.EVENT_WRITE | selectors.EVENT_READ
        self.sel.register(conn, event, data=datas)
    ...
    def handle_io_device_events(self, io_device, event_mask):
        # TODO: Implement the above functionality
        my_sock = io_device.fileobj
        my_sock_data = io_device.data

        if event_mask & selectors.EVENT_READ:
            msg = my_sock.recv(1024)
            if msg:
                my_sock_data.write_buffer += msg
                self.handle_messages(io_device, msg)
            else:
                self.sel.unregister(my_sock)
                my_sock.close()

        if event_mask & selectors.EVENT_WRITE:
            if my_sock_data.write_buffer:
                my_sock.send(my_sock_data.write_buffer)
                my_sock_data.write_buffer = b''
    ...
    # ignore_host_id prior to sending the message.
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        # TODO: Implement the above functionality
        for adj_connections in self.adjacent_server_ids:
            if adj_connections != ignore_host_id:
                self.send_message_to_host(adj_connections, message)
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        # TODO: Implement the above functionality
        for adj_connections in self.adjacent_user_ids:
            if adj_connections != ignore_host_id:
                self.send_message_to_host(adj_connections, message)
    ...
    def handle_server_registration_message(self, io_device, message):
        # TODO: Implement the above functionality
        msg = ServerRegistrationMessage(message)
        if msg.source_id in self.hosts_db:
            message_type = 0x01
            sourceId = msg.source_id
            destId = 0
            messageCode = 0x02
            content = "A machine has already registered with ID" + sourceId
            contentLength = len(content)
            statUpdateMsg = pack("!BIIH" + str(contentLength) + "s", message_type, sourceId, destId, messageCode,
                                 contentLength, bytes(content, 'ascii'))
            self.handle_messages(io_device, statUpdateMsg)
            return
        newServer = ServerData(msg.source_id, msg.server_name, msg.server_info)
        newServer.first_link_id = self.id
        self.hosts_db[msg.source_id] = newServer

        # figure out first link and if servers are adjacent
        for connections in self.hosts_db:
            if isinstance(self.hosts_db[connections].data, ServerData()) and connections != self.id:
                msg_type = 0x00
                srcId = self.hosts_db[connections].data.id
                lastHop = self.id
                nameLength = len(self.server_name)
                infoLength = len(self.server_info)
                info = self.server_info
                name = self.server_name
                msg = pack("!BIIBH" + str(nameLength) + "s" + str(infoLength) + "s", msg_type, srcId, lastHop,
                           nameLength,
                           infoLength, bytes(name, 'ascii'), bytes(info, 'ascii'))
                #how to send to new server?
                self.handle_messages(io_device, msg)
            else:
                msg_type = 0x80
                srcId = self.hosts_db[connections].data.id
                ...
                #how to send to new server?
                self.handle_messages(io_device, msg)
            #how to inform rest of connections of new server
    ...
    def handle_status_message(self, select_key, message):
        # TODO: Implement the above functionality
        msg = StatusUpdateMessage(message)
        if msg.destination_id == self.id or msg.destination_id == 0:
            self.status_updates_log.append(msg.content)
        else:
    ...
    def handle_client_chat_message(self, select_key, message):
        # TODO: Implement the above functionality
        msg = ClientChatMessage(message)
        if msg.destination_id in self.hosts_db:
            self.handle_messages(select_key, message)
        else:
            srcId = self.id
            msgString = "Unknown ID " + msg.destination_id
            msgLength = len(msgString)
            statMsg = pack("!BIIBI" + str(msgLength) + "s", 0x81, srcId, 0, 0x01, msgLength,
                           bytes(msgString, 'ascii'))
            self.handle_messages(select_key, message)
    ...
    def handle_client_quit_message(self, io_device, message):
        # TODO: Implement the above functionality
        if io_device.data.id in self.hosts_db:
            for ids in self.hosts_db:
                if ids != io_device.data.id:
                    self.handle_messages(io_device, message)
        del self.hosts_db[io_device.data.id]
        if io_device.data.id in self.adjacent_user_ids:
            del self.adjacent_user_ids[io_device.data.id]
