...
class ARPHeader(LayerHeader):
    def __init__(self, pkt):
        ...
        #2s = H, 4s = I, 1s = B
        self.hardware_type, self.protocol_type = struct.unpack("!H H", pkt[:4])
        self.hardware_address_len, self.protocol_address_len = struct.unpack("!4x B B", pkt[:6])
        self.opcode, self.sender_hardware_address = struct.unpack("!6x H 6s", pkt[:14])
        self.sender_protocol_address, self.target_hardware_address = struct.unpack("!14x I 6s", pkt[:24])
        self.target_protocol_address = struct.unpack("!24x I", pkt[:header_length])
        #self.target_protocol_address = int(self.target_protocol_address[0])
        print(self.target_protocol_address)
        self.target_protocol_address = self.target_protocol_address[0]
        print(self.target_protocol_address)
